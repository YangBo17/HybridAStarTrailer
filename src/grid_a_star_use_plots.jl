...
function main()
    ...
    fig_astar = plot(ox, oy, color=:black, seriestype=:scatter, label="obstacles", ratio=:equal)
    plot!(fig_astar, [sx], [sy], color=:red, seriestype=:scatter, marker=:star5, label="start")
    plot!(fig_astar, [gx], [gy], color=:red, seriestype=:scatter, marker=:circle, label="goal")
    plot!(fig_astar, rx, ry, color=:red, linestyle=:solid, label="A* path")
